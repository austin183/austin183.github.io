 ...
 *
 * Default: CollageMaker/MyComponents/*Test.html (all matching files)
 */
...
function checkServer() {
    return new Promise((resolve) => {
        const options = {
            hostname: 'localhost',
            port: SERVER_PORT,
            path: '/CollageMaker/MyComponents/',
            method: 'HEAD',
            timeout: 3000
        };
...
function findTestFiles() {
    ...
    return files
        ...
        .map(file => `CollageMaker/MyComponents/${file}`);
}

// Check if a test file exists on the server
// Always resolves to default path: CollageMaker/MyComponents/<filename>
async function testFileExists(testPath) {
    ...
    const url = 'CollageMaker/MyComponents/' + fileName;
