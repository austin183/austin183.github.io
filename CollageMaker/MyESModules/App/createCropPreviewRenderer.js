 ...
 * The renderer supports multiple canvas IDs via domIds configuration.
 * Currently only cropPreviewCanvas (desktop sidebar) is configured.
 * Additional canvases can be added by extending domIds — the renderer
 * iterates over all configured IDs (see _scheduleCropPreviewRender).
 */
...
export function createCropPreviewRenderer(base, domIds = {}) {
    ...
    /**
     * Renders the crop preview onto a single canvas.
     * Skips rendering if the canvas is not visible (display: none or zero dimensions)
     * to avoid unnecessary work on hidden canvases (e.g., desktop sidebar canvas
     * when only the mobile bottom sheet is visible).
     * @param {Object} vm — Vue instance with reactive state
     * @param {HTMLCanvasElement} canvas — Target canvas element
     */
    function _renderCropPreviewOnCanvas(vm, canvas) {
        // Skip hidden canvases to avoid rendering overhead on mobile
     ...
     * Renders to all configured crop preview canvases (desktop sidebar
    ...
    function _scheduleCropPreviewRender(vm) {
        ...
        requestAnimationFrame(() => {
            ...
            // Collect all configured crop preview canvas IDs
            const canvasIds = [];
            if (ids.cropPreviewCanvas) canvasIds.push(ids.cropPreviewCanvas);
            if (ids.bsCropPreviewCanvas) canvasIds.push(ids.bsCropPreviewCanvas);

            for (const canvasId of canvasIds) {
                const canvas = document.getElementById(canvasId);
                if (canvas) {
                    _renderCropPreviewOnCanvas(vm, canvas);
                }
            }
        });
    }
